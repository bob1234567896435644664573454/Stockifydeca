-- Set scheduled competition jobs default mode to rules_compliance_weighted.
...
begin
  ...
  begin perform cron.unschedule('stockify_engine_tick'); exception when others then null; end;
  begin perform cron.unschedule('stockify_competition_jobs'); exception when others then null; end;
  begin perform cron.unschedule('stockify_report_jobs'); exception when others then null; end;
  begin perform cron.unschedule('stockify_market_data'); exception when others then null; end;
  ...
  perform cron.schedule(
    ...
    format($f$
      select net.http_post(
        ...
        body := '{"mode":"rules_compliance_weighted"}'::jsonb
      );
